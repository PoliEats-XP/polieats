...
export class OrderService {
  ...
  async updateItemsQuantities(items: { id: number, quant: number }[]): Promise<void> {
    ...
    if (itemsToUpdate.length > 0) {
      await Promise.all(
        itemsToUpdate.map(async (item) => {
          await OrderRepository.updateItemQuantity(orderId, item.id.toString(), item.quant);
        })
      );
    }
  ...
  async getOrderSummary(): Promise<string> {
    ...
    const items = order.items.map((item: { itemId: unknown; quantity: unknown; }) => {
      const menuItem = menu.find(m => m.id === Number(item.itemId));
      return menuItem ? `- ${menuItem.nome} (quantity: ${item.quantity})` : null;
    }).filter(Boolean);
  ...
  async getCurrentOrder(): Promise<Order> {
    ...
    const itemsWithNames = order.items.map((item: { itemId: number; quantity: number; }) => {
      const menuItem = menu.find(m => m.id === Number(item.itemId));
      return {
        id: item.itemId,
        quant: item.quantity,
    ...
    return {
      items: itemsWithNames.reduce((acc: { [x: string]: { quant: number; name: string; }; }, item: { id: string | number; quant: number; name: string; }) => {
